...
class PurchaseRequster(models.Model):
    ...
    def create(self, vals):
        if vals.get('name_seq', 'New') == 'New':
            vals['name_seq'] = self.env['ir.sequence'].next_by_code('purchase.requster') or '/'
    ...
    date_order = fields.Datetime('Satin Alma Taleb Tarihi', required=True, index=True, copy=False,
    ...
    purchase_order_id = fields.One2many('purchase.order', 'purchase_requester_id', string='Purchase Order Reference', states={'acc': [('readonly', True)]})
